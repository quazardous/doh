...
Follow `/rules/github-operations.md` to ensure we're not syncing to the DOH template:
...
sed '1,/^---$/d; 1,/^---$/d' .doh/epics/$ARGUMENTS/epic.md > /tmp/epic-body-raw.md
...
if [ "$task_count" -lt 5 ]; then
  ...
  for task_file in .doh/epics/$ARGUMENTS/[0-9][0-9][0-9].md; do
...
sed -i.bak "/^github:/c\github: $epic_url" .doh/epics/$ARGUMENTS/epic.md
sed -i.bak "/^updated:/c\updated: $current_date" .doh/epics/$ARGUMENTS/epic.md
rm .doh/epics/$ARGUMENTS/epic.md.bak
```
...
cp .doh/epics/$ARGUMENTS/epic.md .doh/epics/$ARGUMENTS/epic.md.backup
...
' .doh/epics/$ARGUMENTS/epic.md.backup > .doh/epics/$ARGUMENTS/epic.md

# Clean up
rm .doh/epics/$ARGUMENTS/epic.md.backup
rm /tmp/tasks-section.md
```

### 6. Create Mapping File

Create `.doh/epics/$ARGUMENTS/github-mapping.md`:
```bash
# Create mapping file
cat > .doh/epics/$ARGUMENTS/github-mapping.md << EOF
...
# Add sync timestamp
echo "" >> .doh/epics/$ARGUMENTS/github-mapping.md
echo "Synced: $(date -u +"%Y-%m-%dT%H:%M:%SZ")" >> .doh/epics/$ARGUMENTS/github-mapping.md
```
...
Next steps:
  - Start parallel execution: /doh:epic-start $ARGUMENTS
  - Or work on single issue: /doh:issue-start {issue_number}
