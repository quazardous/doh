...
This command creates the initial project context documentation in `.doh/context/` by analyzing the current project state and establishing comprehensive baseline documentation.
...
- If directory exists and has files:
  - Count existing files: `ls -1 .doh/context/*.md 2>/dev/null | wc -l`
...
- If `.claude/` doesn't exist, create it: `mkdir -p .doh/context/`
- Verify write permissions: `touch .doh/context/.test && rm .doh/context/.test`
...
**Common Issues:**
- **No write permissions:** "❌ Cannot write to .doh/context/. Check permissions."
...
📁 Created context in: .doh/context/
...
- Target directory: `.doh/context/` (create if needed)
