...
if [ -d ".doh/prds" ]; then
  echo "📄 PRDs:"
  results=$(grep -l -i "$query" .doh/prds/*.md 2>/dev/null)
  if [ -n "$results" ]; then
...
if [ -d ".doh/epics" ]; then
  echo "📚 Epics:"
  results=$(find .doh/epics -name "epic.md" -exec grep -l -i "$query" {} \; 2>/dev/null)
  if [ -n "$results" ]; then
    for file in $results; do
...
if [ -d ".doh/epics" ]; then
  echo "📝 Tasks:"
  results=$(find .doh/epics -name "[0-9]*.md" -exec grep -l -i "$query" {} \; 2>/dev/null | head -10)
  if [ -n "$results" ]; then
    for file in $results; do
      epic_name=$(basename $(dirname "$file"))
