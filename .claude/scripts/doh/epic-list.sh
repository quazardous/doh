...
[ ! -d ".doh/epics" ] && echo "📁 No epics directory found. Create your first epic with: /doh:prd-parse <feature-name>" && exit 0
[ -z "$(ls -d .doh/epics/*/ 2>/dev/null)" ] && echo "📁 No epics found. Create your first epic with: /doh:prd-parse <feature-name>" && exit 0
...
tasks=$(find .doh/epics -name "[0-9]*.md" 2>/dev/null | wc -l)
