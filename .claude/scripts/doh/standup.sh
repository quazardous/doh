...
for updates_dir in .doh/epics/*/updates/*/; do
...
echo ""
echo "📊 Quick Stats:"
total_tasks=$(find .doh/epics -name "[0-9]*.md" 2>/dev/null | wc -l)
open_tasks=$(find .doh/epics -name "[0-9]*.md" -exec grep -l "^status: *open" {} \; 2>/dev/null | wc -l)
closed_tasks=$(find .doh/epics -name "[0-9]*.md" -exec grep -l "^status: *closed" {} \; 2>/dev/null | wc -l)
