...
Restructurer le système de cache de linting en déplaçant `.lint-cache/` vers `.cache/linting/` et implémenter les caches détaillés par type d'erreur prévus dans DD108.
...
### Phase 1: Migration du Cache Existant

**Migration des chemins** :
- `.lint-cache/error-files.txt` → `.cache/linting/error-files.txt`
...
### Phase 3: Scripts de Gestion Avancée
...
- [ ] Tester migration avec cache existant
...
- ⚡ **Performance**: Traitement par type sans re-scan complet
...
- [ ] Compatibilité avec workflow `/dd:lint` existant

## Brainstorm: Linters Analysis et Intégration

### Linters Utilisés dans le Projet
...
- **Management**: `manager_markdownlint_plugin.sh`
...
- **Management**: `manager_prettier_plugin.sh`
...
- **Management**: `manager_codespell_plugin.sh`
...
- **Management**: `manager_shellcheck_plugin.sh`
...
├── manager_markdownlint_plugin.sh      # Gestionnaire markdownlint
├── manager_prettier_plugin.sh          # Gestionnaire prettier  
├── manager_codespell_plugin.sh         # Gestionnaire codespell
├── manager_shellcheck_plugin.sh        # Gestionnaire shellcheck
│
...
./manager_markdownlint_plugin.sh --enable md036-emphasis-headers

# Prettier  
./manager_prettier_plugin.sh --install smart-line-breaks
./manager_prettier_plugin.sh --disable code-formatting

# Codespell
./manager_codespell_plugin.sh --install project-dictionary
./manager_codespell_plugin.sh --status ignore-patterns

# ShellCheck
./manager_shellcheck_plugin.sh --install quoting-fixes
```

### Intégration avec /dd:lint
...
Cette restructuration prépare le système pour les plugins intelligents prévus dans DD108, en fournissant les données structurées nécessaires pour des suggestions précises.
