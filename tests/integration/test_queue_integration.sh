...
# Updated to use actual message-queue.sh API
...
test_queue_initialization() {
    local queue_dir
    queue_dir="$(ensure_queue_dir "$DOH_TEST_QUEUE_NAME")"
...
test_message_creation() {
    ...
    local message
    message="$(create_renumber_message "task" "test_task" "001" "002" "conflict")"
    
    _tf_assert_command_succeeds "test -n '$message'" "Should create renumber message"
    
    # Validate message format
    _tf_assert_command_succeeds "validate_message '$message'" "Message should be valid"
}

test_queue_message() {
    ensure_queue_dir "$DOH_TEST_QUEUE_NAME"
    ...
    local message_id
    message_id="$(queue_message "$DOH_TEST_QUEUE_NAME" "$message")"
    
    _tf_assert_command_succeeds "test -n '$message_id'" "Message ID should be generated"
    
    # Check message was queued
    local queue_dir
    queue_dir="$(get_queue_dir "$DOH_TEST_QUEUE_NAME")"
...
test_get_message() {
    ...
    # Create and queue a message
    local message
    message="$(create_renumber_message "epic" "test_epic" "003" "004" "gap_filling")"
    ...
    retrieved_message="$(get_message "$DOH_TEST_QUEUE_NAME" "$message_id")"
...
test_list_messages() {
    ...
    local msg1 msg2
    msg1="$(create_renumber_message "task" "task1" "001" "002" "conflict")"
    msg2="$(create_renumber_message "task" "task2" "003" "004" "conflict")"
    
    local id1 id2
    id1="$(queue_message "$DOH_TEST_QUEUE_NAME" "$msg1")"
    id2="$(queue_message "$DOH_TEST_QUEUE_NAME" "$msg2")"
    
    # List pending messages
    local messages
    messages="$(list_messages "$DOH_TEST_QUEUE_NAME" "pending")"
...
test_message_status_change() {
    ...
    # Create and queue a message
    local message
    message="$(create_renumber_message "task" "status_test" "001" "002" "conflict")"
    
    local message_id
    message_id="$(queue_message "$DOH_TEST_QUEUE_NAME" "$message")"
    
    # Change status to OK
    _tf_assert_command_succeeds "set_message_status '$DOH_TEST_QUEUE_NAME' '$message_id' '.ok'" "Should set message status to OK"
    
    # Check message appears in OK list
    local ok_messages
    ok_messages="$(list_messages "$DOH_TEST_QUEUE_NAME" "ok")"
...
test_process_message() {
    ...
    # Create and queue a message
    local message
    message="$(create_renumber_message "task" "process_test" "001" "002" "conflict")"
    
    local message_id
    message_id="$(queue_message "$DOH_TEST_QUEUE_NAME" "$message")"
    
    # Process the message
    _tf_assert_command_succeeds "process_message '$DOH_TEST_QUEUE_NAME' '$message_id'" "Should process message"
...
test_queue_statistics() {
    ...
    local msg1 msg2
    msg1="$(create_renumber_message "task" "stats1" "001" "002" "conflict")"
    msg2="$(create_renumber_message "epic" "stats2" "003" "004" "gap_filling")"
    
    queue_message "$DOH_TEST_QUEUE_NAME" "$msg1"
    queue_message "$DOH_TEST_QUEUE_NAME" "$msg2"
    
    # Get statistics
    local stats
    stats="$(get_queue_stats "$DOH_TEST_QUEUE_NAME")"
...
test_purge_processed_messages() {
    ensure_queue_dir "$DOH_TEST_QUEUE_NAME"
    
    # Create and process a message
    local message
    message="$(create_renumber_message "task" "purge_test" "001" "002" "conflict")"
    
    local message_id
    message_id="$(queue_message "$DOH_TEST_QUEUE_NAME" "$message")"
    
    # Mark as processed
    set_message_status "$DOH_TEST_QUEUE_NAME" "$message_id" ".ok"
    
    # Test purge function (should not purge recent messages)
    _tf_assert_command_succeeds "purge_processed_messages '$DOH_TEST_QUEUE_NAME' 0" "Should run purge function"
}
