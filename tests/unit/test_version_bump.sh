...
test_increment_version_patch() {
    local result
    result=$(increment_version "0.1.0" "patch")
...
test_increment_version_minor() {
    local result
    result=$(increment_version "0.1.0" "minor")
...
test_increment_version_major() {
    local result
    result=$(increment_version "0.1.0" "major")
...
test_increment_version_invalid() {
    local result
    result=$(increment_version "0.1.0" "invalid" 2>/dev/null)
    local exit_code=$?
...
test_set_project_version() {
    ...
    _tf_assert_equals 0 $exit_code "set_project_version should succeed"
...
test_get_file_version() {
    ...
    local version
    version=$(get_file_version ".doh/test.md")
    local exit_code=$?
    
    _tf_assert_equals 0 $exit_code "get_file_version should succeed"
...
test_set_file_version() {
    ...
    set_file_version ".doh/test.md" "0.2.0" > /dev/null
    local exit_code=$?
    
    _tf_assert_equals 0 $exit_code "set_file_version should succeed"
    
    local new_version
    new_version=$(get_file_version ".doh/test.md")
...
test_bump_file_version() {
    ...
    local new_version
    new_version=$(bump_file_version ".doh/another.md" "minor")
    local exit_code=$?
    
    _tf_assert_equals 0 $exit_code "bump_file_version should succeed"
    _tf_assert_equals "0.2.0" "$new_version" "New file version should be 0.2.0"
    
    local file_version
    file_version=$(get_file_version ".doh/another.md")
...
test_validate_version() {
    ...
    _tf_assert_command_fails validate_version "1.0.0.0" "Four-part version should fail"
    _tf_assert_command_fails validate_version "" "Empty version should fail"
}
...
test_version_bump_workflow() {
    ...
    set_file_version ".doh/test.md" "$new_version" > /dev/null
    set_file_version ".doh/another.md" "$new_version" > /dev/null
    
    # Verify file versions are updated
    local test_file_version
    test_file_version=$(get_file_version ".doh/test.md")
    _tf_assert_equals "$new_version" "$test_file_version" "Test file version should be updated"
    
    local another_file_version
    another_file_version=$(get_file_version ".doh/another.md")
